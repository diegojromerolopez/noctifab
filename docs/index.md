...
- **Real-Time Visual Web Dashboard (`noctifab dashboard -w` / `noctifab start -w`)**: Modern embedded web UI featuring live topological task DAG rendering, syntax-highlighted code diffs, real-time Server-Sent Events (SSE) telemetry stream, prompt order input bar, and flow pause/resume controls.
