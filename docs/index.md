...
- **Global Task DAG Scheduler & Cross-Story Pipelining**: Evaluates dependencies at the fine-grained task level across the entire project using globally unique identifiers (`US-XXX-TASK-YYY`). Downstream stories execute tasks concurrently the moment prerequisite foundation or interface tasks merge into `main`, eliminating false story serialization and cutting overall wall-clock time by >50%.
...
- **Topological Task Scheduling & Concurrency**: Automatically constructs a Directed Acyclic Graph (DAG) of task dependencies within and across stories, running independent tasks concurrently in isolated Git worktrees.
...
- **Legacy Codebase Stabilization & Greenfield Detection**: Automatically distinguishes legacy codebases from greenfield starter projects (`ScanLegacyFiles`, `IsGreenfieldWorkspace`), ignoring dependency manifests, lockfiles, and stubs (< 5 lines). Dynamically injects `roadmap/user-stories/US-001.md` characterization testing mandates only when substantial pre-existing legacy code ($\ge 50$ LOC) is present.
- **Shared Dependency Worktree Caches**: Eliminates redundant package installations and recompilation overhead in parallel worktrees via `< 1ms` symlink projection (`node_modules`, `.venv`, `vendor`, `gradle/`, `.mvn/`) and universal compiler cache redirection (`CARGO_TARGET_DIR`, `GOCACHE`, `GRADLE_USER_HOME`, `MAVEN_OPTS`, `CCACHE_DIR`).
- **Unified Pipelined Task Synthesis / Co-Synthesis Mode**: Streamlines generation latency by synthesizing source code and comprehensive test coverage in a single generator pass (`single_pass_co_synthesis`, `co_synthesis`), with zero-token auto-formatting (`stageAndCommit`) and pre-test anti-stub auditing.
- **Pre-Flight `.gitignore` Guardrails & Build Artifact Isolation**: Protects the workspace against runaway file indexing and Git repository pollution by automatically verifying and synthesizing language-agnostic build artifact and cache ignore rules (`EnsureProjectGitignore`, `IsPathExcluded`).
- **String-Literal Aware Code Fence Parsing**: Robust lexical parser tracking string boundaries and nesting depth, preserving embedded code blocks inside JSON payloads and eliminating `"no valid JSON object detected"` parse errors.
- **Process-Aware Git Lock Cleaning & Rebase Resilience**: Verifies process liveness via signal 0 before removing locks and enforces a safe 60-second fallback age threshold to prevent race conditions during concurrent git operations.
