...
- **Always-On Background Standby Daemon & Real-Time Visual Web Dashboard (`noctifab start -w -o` / `noctifab dashboard -w -o`)**: Modern embedded web UI featuring auto-browser launch (`-o` / `--open`), zero-idle-CPU standby mode (`STANDBY 🟢`), live topological task DAG rendering, syntax-highlighted code diffs, real-time Server-Sent Events (SSE) telemetry stream, prompt order input bar, desktop alerts, and flow pause/resume controls.
