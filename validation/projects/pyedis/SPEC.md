...
**Linter Gate (REQUIRED):** The project MUST pass `make lint` with ZERO findings before the test gate is considered green. Both `ruff check app tests` (all default rules) and `mypy --strict app` are mandatory — no `# noqa` suppressions of style rules and no `# type: ignore` that hides typing issues.
...
- **Python 3.10+ Type Hints:** Every function, method, dataclass, and module must be strictly annotated using built-in generic collections (`dict`, `list`, `tuple`, `set`). Do NOT import `Dict`, `List`, `Tuple`, `Set` from `typing`. `mypy --strict app` must pass with zero errors.
- **SOLID & Domain-Driven Design (DDD):**
  - `app/resp.py`: Pure protocol encoding and streaming decoding without networking or storage logic.
  - `app/store.py`: In-memory storage, expiration logic, and clock abstractions without socket or AOF dependencies.
  - `app/commands.py`: Command routing, syntax parsing, and RESP envelope generation.
  - `app/persistence.py`: AOF append logger and replay engine.
  - `app/main.py`: Composition root, `asyncio.start_server` TCP loop, and signal handling.
...
3. **Linting Invariant:** Zero findings under `ruff check app tests` AND `mypy --strict app`.
4. **Verification Criteria:** 100% test pass rate on `make test` (unit + integration) and `make e2e`, with $\ge 95\%$ test coverage across `app/`.
