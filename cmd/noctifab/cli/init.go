...
`
	...
	// 8. Generate roadmap/US-001.md template only when SPEC.md was also just created
	...
	if err := os.MkdirAll(roadmapDir, 0755); err != nil {
		return false, fmt.Errorf("failed to create roadmap directory: %w", err)
	}

	us1Path := filepath.Join(roadmapDir, "US-001.md")
	if createdSpec {
