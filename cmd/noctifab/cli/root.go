...
func init() {
	...
	RootCmd.PersistentFlags().Int("occ-max-retries", 20, "Maximum number of reload-modify-retry iterations on version conflicts")
	RootCmd.PersistentFlags().String("occ-backoff-base", "200ms", "Base delay time duration for OCC lock retry backoff")
