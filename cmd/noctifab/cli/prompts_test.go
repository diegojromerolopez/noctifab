...
func TestPromptsListCommand(t *testing.T) {
	t.Run("when no overrides exist it lists all 14 actions as embedded", func(t *testing.T) {
		ws := t.TempDir()
		out, err := runPromptsCmd(t, ws, "list")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		for _, needle := range []string{"product_manager", "planner", "tester", "generator", "implement", "write_breadth_first"} {
			if !strings.Contains(out, needle) {
				t.Errorf("expected %q in list output, got:\n%s", needle, out)
			}
		}
		if strings.Count(out, "embedded") != 14 {
			t.Errorf("expected 14 embedded entries, got %d:\n%s", strings.Count(out, "embedded"), out)
		}
	})
...
func TestPromptsInitCommand(t *testing.T) {
	...
	t.Run("when initializing everything it writes all 14 templates", func(t *testing.T) {
		ws := t.TempDir()
		out, err := runPromptsCmd(t, ws, "init")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if strings.Count(out, "created ") != 14 {
			t.Errorf("expected 14 created files, got:\n%s", out)
		}
	})
