...
- **Self-Patching Loop (AUT-601)**: Added `SelfUpdateManager` with `BuildAndTest` that copies the repo to a temp sandbox, validates patch paths (rejects `go.mod`/`go.sum` and paths outside `cmd/noctifab/` or `pkg/`), applies patches, runs `go build` and `go test ./pkg/...`, then replaces the binary on success.
