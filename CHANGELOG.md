...
### Changed
- **Eliminated Mechanical Compaction Across Validation Configurations (`validation/projects/**/.noctifab/config.yaml`)**:
  - Replaced legacy `compaction: caveman` with `compaction: none` across all 20 validation projects (`actodis`, `auth-vault`, `buffonstream`, `calculator`, `djanban`, `dotchess`, `echo`, `fortune`, `frontpunch`, `jpacioli`, `ninline`, `notebook`, `ocalogue`, `pyedis`, `searchthedocs`, `stricc`, `t4`, `thredis`, `todo-cli`, `wc`) and `/Users/diegoj/repos/pyedis/.noctifab/config.yaml`.
  - Guarantees 100% verbatim prompt delivery with zero mechanical text filtering, keyword stripping, or syntactic distortion.
...
### Fixed
- **Native E2E Docker Leaks Guard (`pkg/services/e2e_detector.go`)**:
  - Implemented `isMakefileE2ENative` to verify that `make e2e` does not delegate internally to `docker` or `docker compose`.
  - When `sandbox.e2e.mode: native` is enabled, any Docker-delegating Makefile targets are automatically bypassed, falling back to hermetic host test runners (`pytest`, `cargo`, `go test`, `npm`).
...
- **Product Manager Pre-Flight QA Auditor & Story Sanitizer**:
  - Implemented `StoryQAAuditor` and `StorySanitizer` (`pkg/services/story_qa_auditor.go`, `pkg/services/story_sanitizer.go`) to validate and normalize user stories generated by the PM before planner decomposition.
  - Automatically verifies public API definitions, verifies executable/test commands, flags underspecified stories, strips bad markdown formatting/placeholders, and triggers autonomous repair if critical validation invariants are missing.
- **FallbackAgent `clear_agent` Action & Inconsistency Guard**:
  - Added `clear_agent` unblocker action in `pkg/services/unblocker_prompt.go` and `pkg/services/fallback_agent.go` to safely clear orphaned or inconsistent agent registrations without discarding task progress or incrementing retries.
  - Added protective fallback diversion in `fallback_agent.go` to prevent `reset_task` from misidentifying agent inconsistencies as task failures.
  - Updated `ResetTaskCmd` in `pkg/services/unblocker_commands.go` to respect configured `MaxRetries`.
- **Adaptive LLM Rate Limiting & Router Cooldown Isolation**:
  - Implemented `RateLimiter` (`pkg/infrastructure/llm/router_ratelimit.go`) for tracking provider RPM/TPM ceilings and isolating 429 quota exhaustion events from permanent model evictions.
- **Deterministic Formatter & Edit File Helper Refactoring**:
  - Refactored `pkg/services/edit_file_helper.go` and `pkg/services/deterministic_formatter.go` to maintain strict adherence to the 500 LOC per file limit while providing deterministic diff generation and file edits.

### Fixed
- **Fast-Path Syntax Pre-Gating Directory Bypass**:
  - Fixed directory execution bug in `pkg/services/syntax_check_hook.go` where syntax checkers with `{file}` templates (e.g. `ruby -c {file}`, `python3 -m py_compile {file}`, `gcc -fsyntax-only {file}`) were invoked on workspace directories, causing spurious runtime failures and retry starvation.
- **Claude Thinking Token Starvation Recovery**:
  - In `pkg/infrastructure/llm/anthropic.go`, raised default `max_tokens` from 4096 to 8192 and enforced `max_tokens >= budget_tokens + 4096` when extended thinking is enabled.
  - Implemented automatic token doubling (up to 32768) and retry when Claude returns `stop_reason: "max_tokens"` with zero text output due to thinking exhaustion.
- **Circuit Breaker False Positives**:
  - Hardened circuit breaker pattern matching in `pkg/services/circuit_breaker.go` to prevent transient linter output from erroneously tripping breaker states.
...
### Fixed
- **Multi-Story Task Scoping & Premature Story Finalization**:
  - Fixed `getStoryTasks` in `cmd/noctifab/cli/start_story_executor.go` to avoid falling back to returning all `state.Tasks` when a newly spawned story in a multi-story project has not yet planned its tasks.
  - Prevents sequential user stories (`US-002`, `US-003`) from mistakenly evaluating `allStoryTasksFinished` as true against the previous story's finished tasks and skipping execution without planning or generating code.
...
### Fixed
- **`.noctifab/.gitignore` data directory coverage**: `noctifab init` now ignores the entire `data/` folder instead of only `data/noctifab.db`, ensuring SQLite WAL (`noctifab.db-wal`) and shared-memory (`noctifab.db-shm`) side-car files are also excluded from version control.
...
### Fixed
- **Skip Default `secrets.yaml` Template Generation When Global `$HOME/.noctifab/secrets.yaml` Exists**:
  - `EnsureWorkspaceInitializedWithProfile` (called during `noctifab init` and `noctifab start`) now checks `config.HasGlobalSecrets()` before generating a project-level `.noctifab/secrets.yaml` template with empty credentials.
  - When global credentials exist in `$HOME/.noctifab/secrets.yaml`, Noctifab avoids polluting the project directory with redundant empty secrets and adjusts start notification messaging accordingly.
...
- **`jpacioli` Java + Spring Boot Validation Project (`validation/projects/jpacioli/`)**:
  ...
  - Implemented with Java 21, Spring Boot 3.3+, Full Event Sourcing (ES with `event_store` single source of truth and point-in-time time-travel balance reconstruction), CQRS with materialized read-model projections (`account_balances_view`, `account_statements_view`, `trial_balance_view`), and PostgreSQL.
  - Implemented Stateless JWT Bearer Authentication and fine-grained Role-Based Access Control (RBAC) permissions (`ROLE_SUPERADMIN`, `ROLE_ADMIN`, `ROLE_ACCOUNTANT`, `ROLE_AUDITOR`, `ROLE_OPERATOR` with `accounts:*`, `transactions:*`, `reports:*`, `events:*`, and `users:*` authority scopes).
  ...
  - Added `eclipse-temurin:21-jdk-alpine` multi-toolchain Dockerfile, `docker-compose.e2e.yml`, and comprehensive `SPEC.md`.
  - Updated validation matrix and documentation in `validation/projects/TESTING_GUIDE.md`.
