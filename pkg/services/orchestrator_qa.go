...
func (o *Orchestrator) prepareQA(ctx context.Context, state *domain.State, task domain.Task) (domain.StoryContract, *QAReviewResult) {
	...
	// Fallback to task.StoryID or state.StoryContracts if storyPath did not yield a contract
	if len(contract.PublicContracts) == 0 && state != nil {
		targetStoryID := task.StoryID
		if targetStoryID == "" && storyPath != "" {
			targetStoryID = ExtractStoryID(storyPath)
		}
		if targetStoryID != "" {
			for _, sc := range state.StoryContracts {
				if sc.StoryID == targetStoryID && len(sc.PublicContracts) > 0 {
					contract = sc
					break
				}
			}
			if len(contract.PublicContracts) == 0 {
				...
				if matches, err := filepath.Glob(filepath.Join(storiesDir, "*.md")); err == nil {
					for _, m := range matches {
						if ExtractStoryID(m) == targetStoryID {
							if markdown, err := os.ReadFile(m); err == nil {
								if parsed, pErr := ParseStoryContract(relativeStoryPath(state.ProjectPath, m), string(markdown)); pErr == nil {
									contract = parsed
									break
								}
							}
