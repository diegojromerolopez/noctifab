...
func (u *UnblockerAgent) checkAndUnblock(ctx context.Context) {
	...
	for _, s := range stalls {
		...
		// Pre-hard-stop check: If task has stalled 4 times, escalate to Last-Resort Agent
		if s.Task.StallCount == 4 {
			reason := fmt.Sprintf("unblocker: task %s reached stall count 4; escalating to Last-Resort Agent", s.Task.ID)
			directive := fmt.Sprintf("SOVEREIGN REPAIR DIRECTIVE: Task %s has stalled 4 times. Last-Resort Agent must execute with full cross-domain authority.", s.Task.ID)
