...
func TestAgentLoopIterationsConfig(t *testing.T) {
	...
	t.Run("when GeneratorsIterations is zero, the generator loop defaults to 20 turns", func(t *testing.T) {
		llm := &turnCountingLLM{}
		orch, state := newIterationsTestOrchestrator(t, llm, OrchestratorConfig{
			PollInterval: 10 * time.Millisecond,
		})
		orch.RunGeneratorAgent(context.Background(), task, state, nil, "", "implement")
		if got := llm.turns(); got != 20 {
			t.Errorf("expected default 20 generator turns, got %d", got)
		}
	})
	...
	t.Run("when TestersIterations is negative, the tester loop defaults to 20 turns", func(t *testing.T) {
		llm := &turnCountingLLM{}
		orch, state := newIterationsTestOrchestrator(t, llm, OrchestratorConfig{
			PollInterval:      10 * time.Millisecond,
			TestersIterations: -1,
		...
		orch.RunTesterAgent(context.Background(), task, state, nil, "write", "")
		if got := llm.turns(); got != 20 {
			t.Errorf("expected default 20 tester turns, got %d", got)
		}
	})
}
