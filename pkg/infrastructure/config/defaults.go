...
func DefaultConfig() *Config {
	return &Config{
		...
		Roles: RolesConfig{
			...
			Fallback:     RoleSetting{Profile: "fallback", Temperature: 0.1},
			LastResort:   RoleSetting{Profile: "fallback", Temperature: 0.1},
		},
		...
		Fallback: FallbackConfig{
			...
			MaxRetries:        3,
			StallThreshold:    Duration(2 * time.Minute),
			ConflictThreshold: Duration(5 * time.Minute),
			LLMAssessment:     true,
