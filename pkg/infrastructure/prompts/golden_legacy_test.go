...
- STRUCTURED DATA & SCRIPT-FIRST REFLEX: Whenever testing or generating fixtures for tasks dealing with structured data (JSON, YAML, TOML, CSV, schemas, ASTs, tabular tables, or database rows), ASK YOURSELF if writing and running a script (in Python for example) wouldn't be better, faster, and more reliable than manual editing. Use scripts to validate, transform, and verify structured data programmatically.
...
- STRUCTURED DATA & SCRIPT-FIRST REFLEX: Whenever you encounter a task dealing with structured data (JSON, YAML, TOML, CSV, XML, ASTs, schemas, database fixtures, or tabular tables), ASK YOURSELF: "Wouldn't creating and running an automated script (in Python for example) be significantly better, faster, and more reliable than manual line-by-line editing?" If yes, write an executable script (e.g. scripts/transform_data.py or a temporary script), execute it, and let the code handle parsing, validation, and formatting deterministically.
...
- When writing Makefiles or build scripts:
