...
// LoadByID retrieves a specific State domain object from PostgreSQL by its ID.
func (r *PostgresRepository) LoadByID(ctx context.Context, id string) (*domain.State, error) {
...
// LoadAll retrieves all State domain objects from PostgreSQL.
func (r *PostgresRepository) LoadAll(ctx context.Context) ([]*domain.State, error) {
	...
	err = func() error {
		...
		for rows.Next() {
			state, err := r.scanPostgresStateRows(ctx, rows)
			if err != nil {
				return err
			}
...
// scanPostgresStateRow scans a single query row into domain.State.
func (r *PostgresRepository) scanPostgresStateRow(ctx context.Context, row *sql.Row) (*domain.State, error) {
	var state domain.State
	var buildStatusStr string
	var storyStatusStr, storyErrorStr sql.NullString
	err := row.Scan(
...
// scanPostgresStateRows scans the current row from sql.Rows into domain.State.
func (r *PostgresRepository) scanPostgresStateRows(ctx context.Context, rows *sql.Rows) (*domain.State, error) {
	var state domain.State
	var buildStatusStr string
	var storyStatusStr, storyErrorStr sql.NullString
	err := rows.Scan(
...
// loadPostgresStateRelations loads all nested relationships (Stories, Tasks, Clarifications, Actions, Files, ValidationCriteria, ActiveAgents) for a given State.
func (r *PostgresRepository) loadPostgresStateRelations(ctx context.Context, state *domain.State) error {
	...
	rowsCl, err := r.db.QueryContext(ctx,
		...
		FROM clarifications WHERE state_id = $1`, state.ID)
	if err != nil {
		return err
	}
	...
	rowsAc, err := r.db.QueryContext(ctx,
		...
		FROM actions WHERE state_id = $1`, state.ID)
	if err != nil {
		return err
	}
	...
	rowsFi, err := r.db.QueryContext(ctx,
		...
		FROM workspace_files WHERE state_id = $1`, state.ID)
	if err != nil {
		return err
	}
	...
	rowsVc, err := r.db.QueryContext(ctx,
		...
		FROM validation_criteria WHERE state_id = $1`, state.ID)
	if err != nil {
		return err
	}
