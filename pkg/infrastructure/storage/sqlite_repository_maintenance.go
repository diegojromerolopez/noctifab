...
func (r *SQLiteRepository) PruneFinishedStates(ctx context.Context, keepLast int) (int, error) {
	...
	rows, err := tx.QueryContext(ctx,
		...
		WHERE story_status IN (?, ?)
		ORDER BY COALESCE((SELECT MAX(updated_at) FROM tasks WHERE tasks.state_id = state.id), '') DESC, id DESC`,
	...
	placeholders := strings.Repeat("?, ", len(pruneIDs))
...
// task bodies.
func (r *SQLiteRepository) LoadAllSummaries(ctx context.Context) ([]domain.StateSummary, error) {
