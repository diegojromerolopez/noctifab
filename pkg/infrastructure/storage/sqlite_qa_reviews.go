...
func (r *SQLiteRepository) saveQAReviews(ctx context.Context, tx *sql.Tx, state *domain.State) error {
	for _, table := range []string{"qa_findings", "qa_scenarios", "review_phases", "story_contracts"} {
		if _, err := tx.ExecContext(ctx, "DELETE FROM "+table+" WHERE state_id = ?", state.ID); err != nil {
			return err
		}
	}
	for _, contract := range state.StoryContracts {
		...
		if _, err = tx.ExecContext(ctx, `INSERT INTO story_contracts
			(state_id, story_id, source_path, source_sha256, public_contracts) VALUES (?, ?, ?, ?, ?)`,
			state.ID, contract.StoryID, contract.SourcePath, contract.SourceSHA256, string(payload)); err != nil {
			return err
		}
	}
	...
	for _, scenario := range state.QAScenarios {
		...
		if _, err = tx.ExecContext(ctx, `INSERT INTO qa_scenarios
			...
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, scenario.ID, state.ID, scenario.ReviewPhaseID,
			scenario.PublicContractID, scenario.Name, scenario.Fingerprint, string(steps), string(scenario.Status), scenario.Evidence); err != nil {
			return err
		}
	}
	for _, finding := range state.QAFindings {
		if _, err := tx.ExecContext(ctx, `INSERT INTO qa_findings
			...
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, finding.ID, state.ID, finding.ReviewPhaseID,
			finding.TaskID, finding.ArtifactID, finding.ScenarioFingerprint, finding.PublicContractID, finding.Severity,
			finding.Expected, finding.Actual, finding.Evidence, finding.Disposition); err != nil {
			return err
		}
	}
	return nil
...
func (r *SQLiteRepository) loadQAReviews(ctx context.Context, state *domain.State) error {
	rows, err := r.db.QueryContext(ctx, `SELECT story_id, source_path, source_sha256, public_contracts
		FROM story_contracts WHERE state_id = ? ORDER BY created_at, story_id`, state.ID)
	if err != nil {
		return err
	}
	...
	for rows.Next() {
		...
		if err := json.Unmarshal([]byte(payload), &contract.PublicContracts); err != nil {
			return err
		}
	...
	return r.loadSQLiteReviewResults(ctx, state)
}
...
func (r *SQLiteRepository) loadSQLiteReviewResults(ctx context.Context, state *domain.State) error {
	...
	for rows.Next() {
		...
		if err := json.Unmarshal([]byte(manifest), &phase.ArtifactManifest); err != nil {
			_ = rows.Close()
			return err
		}
		if completed.Valid {
	...
	return r.loadSQLiteScenariosAndFindings(ctx, state)
}

func (r *SQLiteRepository) loadSQLiteScenariosAndFindings(ctx context.Context, state *domain.State) error {
	rows, err := r.db.QueryContext(ctx, `SELECT id, review_phase_id, public_contract_id, name, fingerprint, steps, status, evidence
		FROM qa_scenarios WHERE state_id = ? ORDER BY created_at, id`, state.ID)
	if err != nil {
		return err
	}
	...
	for rows.Next() {
		...
		if err := json.Unmarshal([]byte(steps), &scenario.Steps); err != nil {
			_ = rows.Close()
			return err
		}
	...
	rows, err = r.db.QueryContext(ctx, `SELECT id, review_phase_id, task_id, artifact_id, scenario_fingerprint,
		...
		FROM qa_findings WHERE state_id = ? ORDER BY created_at, id`, state.ID)
	if err != nil {
		return err
	}
	defer func() { _ = rows.Close() }()
