...
| Proposal | Target Area | Estimated Speedup | Implementation Effort |
| :--- | :--- | :---: | :---: |
| **1. Parallel DAG Worker Pools** | Task Execution | **2.5x–3.5x** | Medium |
| **2. Tiered LLM Provider Routing** | LLM Latency | **2.0x–3.0x** | Low |
| **3. Parallel 3x Majority-Vote Validation** | Test Verification | **3.0x** | Low |
| **4. Pre-baked Validation Base Images** | Container Setup | **1.5x** | Low |
| **5. Spec-Level Deterministic Mock Clocks** | Retry Avoidance | **2.0x** | Low |
| **6. Native JSON Schema & Parameter Sanitization** | LLM Transport | **1.5x** | Low |
| **7. Implicit Orchestrator Verification** | Pipeline Flow | **1.3x** | Low |
| **8. Aggressive Prompt History Pruning** | Prompt Tokens | **1.4x** | Medium |
| **9. Speculative Next-Task Prefetching on `noop`** | Transition Delay | **1.5x** | Medium |
...
### Proposal 1: Parallel DAG Task Worker Pools
...
### Proposal 2: Tiered LLM Provider Model Routing
...
### Proposal 3: Parallel 3x Majority-Vote Test Validation
...
### Proposal 4: Pre-bake Common Test Dependencies in Base Images
...
### Proposal 5: Spec Invariants for Deterministic Mock Clocks
...
### Proposal 6: Native JSON Schema Enforcement & Parameter Sanitization
...
### Proposal 7: Implicit Orchestrator Verification
...
### Proposal 8: Aggressive Prompt History Pruning on Retries
* **Runtime Issue**: Prompt history grows to `>49KB` on retries, slowing LLM inference.
* **Solution**: On retry turns, pass only target source code + latest test error traceback, discarding intermediate tool history.

### Proposal 9: Speculative Next-Task Prefetching on `noop`
