...
*   **Command Channel Event Loop (`pkg/services/command_channel.go`):** To completely eliminate database OCC lock contentions, worker goroutines dispatch state mutations synchronously via `CommandMailbox.SendSync` (e.g., `StateMutationCmd`, `UpdateTaskStatusCmd`, `SaveActionCmd`, `ReserveTokensCmd`) to a thread-safe Go FIFO channel. The orchestrator runs a single-threaded transaction writer goroutine that processes incoming command payloads from this channel sequentially, executing writes inside isolated, serial transactions and returning write confirmation to calling goroutines.
...
*   `noctifab dashboard`
    Launches the real-time progress dashboard. By default, opens the interactive Terminal User Interface (TUI). When passed `-w` / `--web`, launches the embedded real-time visual web dashboard in the browser (`--port`, `--host`, `--readonly`, `--web-open` to auto-open) with live topological task DAG rendering grouped by user story, interactive Task & Story Definition of Done (DoD) inspector modal, 1-click failure diagnostics and steering hints, workspace changed files tree browser, multi-story roadmap timeline bar, native Web Audio chimes, floating toast notifications, engine telemetry and token breakdown metrics modal, real-time terminal drawer search and tag filtering, downloadable execution summary reports, syntax-highlighted code diffs, real-time Server-Sent Events (SSE) telemetry feed, prompt order input bar, flow pause/resume controls, and Visual Spec Studio with time-travel revision scrubber.
*   `noctifab steer`
